...
- `getting-started/index.html`: the tutorial, adopting Milano in an existing app with the CLI, from `milano init` to a rendered banner, with a platform switch (SwiftUI, Compose, React Native) for the app-side steps. Generated by `tools/getting-started/generate.py` from snippets verified against the SDK (see the README there); edit the sources and regenerate, never the page.
